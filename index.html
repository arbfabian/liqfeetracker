...
<head>
    ...
    <style>
        ...
        /* --- Anpassung für h2 in Position Summary Card --- */
        .position-summary-card h2 {
            margin-top: 0; 
            font-size: 1.15em; /* Beibehalten für Basis-Schriftgröße des h2 */
            color: #34495e;
            border-bottom: none; /* Entfernt, da der Button der Titel ist */
            padding-bottom: 0;  /* Entfernt */
            margin-bottom: 12px;
            width: 100%; 
            text-align: center; 
            display: flex; /* Zentriert den Button-Link */
            justify-content: center;
            align-items: center;
            min-height: 30px; /* Um sicherzustellen, dass der Bereich eine Mindesthöhe hat, auch wenn Text kurz */
        }
        ...
        /* .active-position h2 { color: #27ae60; } *//* Farbe wird vom .position-id-button oder global gesteuert */
        ...
        /* --- Anpassung für .position-title (h2 in Detailansicht) --- */
        .position-title { 
            font-size: 1.4em; /* Basis-Schriftgröße für den Titelbereich */
            margin-bottom: 15px; 
            color: #34495e; 
            border-bottom: none; /* Entfernt, da Button der Titel ist */
            padding-bottom: 0px; /* Entfernt */
            width: 100%; 
            box-sizing: border-box; 
            text-align: center;
            display: flex; /* Zentriert den Button-Link */
            justify-content: center;
            align-items: center;
            min-height: 40px; /* Um sicherzustellen, dass der Bereich eine Mindesthöhe hat */
        }
        ...
        /* --- NEUER CSS-STIL FÜR DEN POSITIONS-ID BUTTON --- */
        .position-id-button {
            display: inline-block; /* Wichtig für Padding etc. */
            padding: 8px 12px;
            background-color: #f0f2f5; /* Leicht anderer Grauton, anpassbar */
            color: #34495e; /* Dunklere Textfarbe für besseren Kontrast */
            ...
            cursor: pointer;
            font-size: 0.9em; /* Relative Schriftgröße zum Elternelement (h2) */
            line-height: normal; /* Stellt sicher, dass der Text vertikal zentriert ist */
            border: 1px solid #d0d7de; /* Dezenter Rand */
            white-space: nowrap; /* Verhindert Umbruch bei langen Texten */
        }

        .active-position .position-id-button { /* Spezifischer Stil für aktive Positionen */
             background-color: #e6f7f0; /* Grüner angehauchter Hintergrund */
             border-color: #a3d9c2;
             color: #20634a;
        }
        .active-position .position-id-button:hover {
             background-color: #d1f0e3;
             color: #174a36;
        }

        .position-id-button:hover {
            background-color: #e4e8ed;
            color: #2c3e50;
            transform: translateY(-1px);
        }

        .position-id-button small { /* Styling für <small> innerhalb des Buttons */
            font-weight: normal;
            color: #7f8c8d; 
            font-size: 0.9em; 
        ...
        @media (max-width: 600px) {
            ...
            .position-title {
                font-size: 1.2em; /* Basis-Schriftgröße für Titelbereich Mobil */
            }
            .position-summary-card h2 {
                font-size: 1.1em; /* Basis-Schriftgröße für Titelbereich Mobil */
            }
            .position-id-button {
                padding: 7px 10px; 
                font-size: 0.85em; /* Relative Schriftgröße für Button Mobil */
            }
            .position-block th, .position-block td {
                padding: 6px; 
                font-size: 0.9em; 
...
<body>
    ...
    <script>
        ...
        async function createPriceChart(canvasId, positionData) {
            // ... (Restlicher Code für createPriceChart bleibt gleich wie in deiner vorherigen Version)
        ...
        async function loadAndDisplayFees() {
            // ... (Restlicher Code für loadAndDisplayFees bleibt gleich wie in deiner vorherigen Version)
            ...
            try {
                ...
                for (const positionIdKey of sortedPositionKeys) {
                    ...
                    let chartHtmlPart = '';
                    if (isActive) { // Chart nur für aktive Positionen in der Übersicht
                    ...
                    if (isActive) { // Chart HTML nur einfügen, wenn aktiv
                    ...
                    // --- ANPASSUNG FÜR BUTTON-LINK IM TITEL ---
                    let titleHtml = `<h2>
                                        <a href="${uniswapLink}" target="_blank" rel="noopener noreferrer" class="position-id-button">
                                            Position ${posNum} 
                                            ${isActive ? '' : '<small>(Archiv.)</small>'}
                                        </a>
                                     </h2>`;
                ...
                if(hasActive){ // Charts nur für aktive Positionen initialisieren
        ...
        function showPositionDetails(positionData, positionIdKey) {
            // ... (Restlicher Code für showPositionDetails bleibt gleich wie in deiner vorherigen Version)
            ...
            // --- ANPASSUNG FÜR BUTTON-LINK IM DETAIL-TITEL ---
            let detailTitleHtml = `<h2 class="position-title">
                                        <a href="${uniswapLink}" target="_blank" rel="noopener noreferrer" class="position-id-button ${isActive ? 'active' : ''}">
            ...
            detailHtml += detailTitleHtml; // Verwendet den angepassten Titel
        ...
        function hidePositionDetails() {
             // ... (Restlicher Code für hidePositionDetails bleibt gleich wie in deiner vorherigen Version)
